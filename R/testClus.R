...
buildAndTest5 <- function(ds, dataset_name, number_dataset,
  ...
  #while(f<=number_folds){
    ...
    setwd(FolderPSplit)
    escolhidos = data.frame(read.csv(paste("fold-",f,"-nh-choosed.csv", sep="")))
    ...
    while(u<=total){
        ...
        setwd(FolderPSplit)
        Knn_NH = data.frame(read.csv(paste("fold-",f,"-nh-choosed.csv", sep="")))
        total_Knn_NH = nrow(Knn_NH)
        Knn_NH2 = Knn_NH[u,]

        setwd(FolderPSplit)
        particoes = data.frame(read.csv(paste("fold-",f,"-nh-partition.csv", sep="")))
        nr = nrow(particoes)
...
##################################################################################################
juntaResultados5 <- function(ds, dataset_name, number_dataset,
                             number_folds, number_cores,
                             folderResults, diretorios){
  ...
  #while(f<=number_folds){
    ...
    while(k<=total_Knn_NH){
      ...
      while(g<=Knn_NH$numberComm){
          ...
          FolderGroup = paste(FolderTestedSplit, "/Group-", g, sep="")
...
avaliaTest2 <- function(ds, dataset_name, number_dataset,
  ...
  #while(f<=10){
    ...
    while(k<=total_Knn_NH){
      ...
      setwd(FolderTestedSplit)
      #setwd(FolderKnn)
      salva3 = paste("Conf-Mat-Fold-", f, "-nh-", k, ".txt", sep="")
      ...
      names(confMatPart) = paste("Fold-", f, "-nh-", k, sep="")
      namae = paste("Split-", f, "-nh-", k,"-Evaluated.csv", sep="")
...
juntaAvaliacoes2 <- function(ds, dataset_name, number_dataset,
  ...
  while(f<=number_folds){
    ...
    while(k<=total_Knn_NH){
      ...
      #FolderTestedPart = paste(FolderTestedSplit, "/nh-", k, sep="")

      ######################################################################################################################
      setwd(FolderTestedSplit)
      #setwd(FolderKnn)
      str = paste("Split-", f, "-nh-", k, "-Evaluated.csv", sep="")
      avaliado = data.frame(read.csv(str))
      names(avaliado)[1] = "medidas"
      avaliadoKnn = cbind(avaliadoKnn, avaliado[,2])
      nomesThreshold[k] = paste("Fold-", f, "-nh-", k, sep="")
...
organizaAvaliacoes2 <- function(ds, dataset_name, number_dataset,
  ...
  while(k<=numCol){
    ...
    nomesFolds = c()
    nomeKnn1 = paste("Evaluated-10Folds-nh-", k, ".csv", sep="")
    nomeKnn2 = paste("Mean-10Folds-nh-", k, ".csv", sep="")
    nomeKnn3 = paste("Median-10Folds-nh-", k, ".csv", sep="")
    nomeKnn4 = paste("SD-10Folds-nh-", k, ".csv", sep="")
